...
## ⚠️ v4.9.3 修正紀錄：「核心模組仍在載入中」導致掃描失敗

**症狀**：v4.9.2 讓真正的錯誤訊息顯示出來後，發現一直卡在同一句話：
「核心模組仍在載入中（首次啟動需要載入 yfinance 等套件...），請稍候再試一次」，
不管自動排程或手動按鈕都一樣，重複出現。

**根本原因**：伺服器剛啟動、或 Render 免費方案閒置一段時間後被喚醒時，
`yfinance`/`pandas` 等套件需要數秒到數十秒才能在背景載入完成。但自動排程
的「第一次掃描」幾乎必然在這個載入窗口內就被觸發，而舊寫法只要發現
「還沒載入完成」就立刻報錯——等於每次冷啟動都保證先失敗一次；更糟的是，
失敗後排程會傻等完整 15 分鐘才重試，所以使用者會長時間卡在這句錯誤訊息，
手動點擊「套用條件並立刻掃描」如果剛好也落在同一個載入窗口內，同樣會
立刻失敗。

**修正內容**：
- `app.py`：`get_core()` 改成「有耐心地等」核心模組載入完成（最多等
  `CORE_LOAD_TIMEOUT_SEC`，預設 60 秒），而不是一發現還沒好就立刻報錯；
  等待時間算在既有的 `SCAN_TIMEOUT_SEC` 看門狗時間內，不會無限期卡住。
  一旦核心模組準備好（正常只需數秒到數十秒），掃描會自動接著往下跑，
  不需要使用者手動重試。
- `app.py`：排程迴圈加了一道保險——萬一真的等超過
  `CORE_LOAD_TIMEOUT_SEC` 仍未就緒（極端情況），會在 20 秒後很快重試，
  而不是照原本的 15 分鐘排程間隔傻等。
- `templates/index.html`：等待核心模組載入時，畫面上的進度訊息會明確
  顯示「等待核心模組（yfinance/pandas）載入完成…」，不會再看起來像卡住
  不動。
