...
def _import_core_once():
    ...
    """
    global _core, _core_import_error, _core_import_started_at
    _core_import_started_at = time.time()
    t0 = _core_import_started_at
...
# ★ 修正：核心模組（yfinance/pandas等）在伺服器剛啟動、或 Render 免費方案
#   閒置後被喚醒時，需要數秒到數十秒才能載入完成。舊寫法一旦發現「還沒
#   載入完成」就立刻報錯，而自動排程的第一次掃描幾乎必然會在這個載入窗口
#   內就被觸發——每次冷啟動都保證先失敗一次，而且失敗後要傻等完整
#   SCAN_INTERVAL_SEC（15分鐘）才會重試，讓使用者長時間看到「上次掃描
#   發生錯誤」。手動按鈕在同一個載入窗口內點擊，也會遇到一模一樣的
#   立即失敗。修正方式：改成「有耐心地等」，最多等 CORE_LOAD_TIMEOUT_SEC
#   秒；一旦核心模組準備好了幾乎馬上就能繼續往下跑，等待時間會直接算在
#   SCAN_TIMEOUT_SEC 看門狗時間內（兩者搭配，不會無限期卡住）。
#   ★ v4.9.4：Render 免費方案的容器在剛醒來時 CPU 配額被大幅節流，
#   讀取 pandas/numpy 這類體積較大的 C 擴充套件可能遠超過 60 秒，
#   所以預設值提高到 180 秒；同時改成每 5 秒回報一次「已等待幾秒」，
#   讓畫面上看得到時間在走、不會誤以為又卡死了。
CORE_LOAD_TIMEOUT_SEC = int(os.environ.get('CORE_LOAD_TIMEOUT_SEC', '180'))
_CORE_WAIT_POLL_SEC = 5

def get_core():
    if not _core_ready_event.is_set():
        waited = 0
        while waited < CORE_LOAD_TIMEOUT_SEC:
            step = min(_CORE_WAIT_POLL_SEC, CORE_LOAD_TIMEOUT_SEC - waited)
            ready = _core_ready_event.wait(timeout=step)
            waited += step
            if ready:
                break
            elapsed = (time.time() - _core_import_started_at) if _core_import_started_at else waited
            _add_log(f'等待核心模組載入中...（已等待 {int(elapsed)} 秒，正常情況數十秒內會完成）', 3)
        if not _core_ready_event.is_set():
            raise RuntimeError(
                f'核心模組載入超過 {CORE_LOAD_TIMEOUT_SEC} 秒仍未完成，'
                f'請檢查 Render 伺服器 log，確認是卡在哪一個 import '
                f'（伺服器 log 會印出「[核心模組] 開始載入...」但沒有印出「✅ 載入完成」或「❌ 載入失敗」）')
...
def _run_one_scan(gen=None):
    ...
    try:
        if not _core_ready_event.is_set():
            _add_log('等待核心模組（yfinance/pandas）載入完成，僅在伺服器剛啟動/剛醒來時發生...', 3)
        else:
            _add_log('初始化核心模組...', 3)
        sc = get_core()
        _add_log('套用篩選參數...', 5)
...
def _scheduler_loop():
    ...
    while True:
        ...
        # ★ 修正：如果失敗原因是核心模組還沒載入完成（正常情況下 get_core()
        # 內部已經會耐心等待，這裡只是保險），就不要傻等完整 15 分鐘才重試，
        # 改成 20 秒後很快再試一次；核心模組準備好之後，之後所有排程就會
        # 恢復正常的 SCAN_INTERVAL_SEC 間隔。
        with _latest_lock:
            core_not_ready = not _core_ready_event.is_set()
            eta_sec = 20 if core_not_ready else SCAN_INTERVAL_SEC
            eta = (_now_tw() + timedelta(seconds=eta_sec)).isoformat()
